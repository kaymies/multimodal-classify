...
Original file is located at
    https://colab.research.google.com/drive/1Wv_0uKZ-gwb4oProSN2E9EQsmQUElYsA
"""
...
# load any pickle that contains a waveform with associated labels here
phys101_seendf = pd.read_pickle("/content/gdrive/My Drive/phys101_seen.pkl")
phys101_unseendf = pd.read_pickle("/content/gdrive/My Drive/phys101_unseen.pkl")

# shuffle the data 
phys101_seendf_shuffled = phys101_seendf.sample(frac=1) 
phys101_unseendf_shuffled = phys101_unseendf.sample(frac=1)
...
train_set = SoundDataset(phys101_seendf_shuffled) # ~80% of data for training
test_set = SoundDataset(phys101_unseendf_shuffled) # ~20% of data for testing
...
# CNNs are used as sound is invariant to translations, and detect higher level
...
# this is a wrapper model that we added to incorporate SoundNet to classify
# object and material types by flattening the output of SoundNet and passing it through
...
class OurSound(nn.Module):
    def __init__(self, soundnet):
        ...
        self.fc_obj = nn.Linear(1024*11, 12) # size is conv7_outchannel x signal_length, output is the number of classes
        self.fc_mat = nn.Linear(1024*11, 5)
    def forward(self, x):
        x = self.soundnet(x) # output of SoundNet
        x = self.flatten(x) # flatten output of SoundNet
...
def train(model, epoch, optimizer):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        data = torch.unsqueeze(data,1) # convert 3 dimensional data to 4 dimensional data that model expects
...
oursound_model = OurSound(soundnet_model)
oursound_model.to(device)

# initializing the optimizer
# to fully leverage the pretrained SoundNet model, we are only updating the
# final layer of SoundNet, as well as the
# weights of the final fully connected layer in our wrapper model
params = list(oursound_model.soundnet.conv7.parameters()) + list(oursound_model.fc_obj.parameters()) + list(oursound_model.fc_mat.parameters())
...
torch.save(oursound_model.state_dict(), '/content/gdrive/My Drive/phys101_soundnet_seenunseen.pth') # save oursound_model weights and biases
