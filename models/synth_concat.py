...
Original file is located at
    https://colab.research.google.com/drive/1SM0qYaCcrarlwsSG-radFznDvtRLOsA3
"""

# combines soundnet and resnet (single frame/image) using simple concatenation
...
class AVDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_image = Image.open(np.asarray(self.df['frame0'])[index])
        ...
        return self.waveforms[index], input_batch, self.labels[index, :]
...
# object and material types by flattening the output of the two models, concatenating, and passing it through
...
class OurSynth(nn.Module):
    def __init__(self, resnet, soundnet):
        ...
        self.fc_obj = nn.Linear(512+1024*11, 12) # size is conv7_outchannel x signal_length, output is the number of classes
        self.fc_mat = nn.Linear(512+1024*11, 5)
        self.soundnet = soundnet
        self.relu = nn.ReLU()
    def forward(self, x_vis, x_sound):
        x_vis = self.features(x_vis)
        x_vis = self.flatten(x_vis) # flatten output
        x_sound = self.soundnet(x_sound) # output of SoundNet
        x_sound = self.flatten(x_sound)
        x = torch.cat((x_vis, x_sound), dim=1) # concatenation
...
def train(model, epoch, optimizer):
    ...
    for batch_idx, (data_sound, data_vis, target) in enumerate(train_loader):
        ...
        data_vis = torch.squeeze(data_vis)
        optimizer.zero_grad()
        ...
        output_obj, output_mat = model(data_vis, data_sound) # separate the output into material, object, scene
...
def test(model, epoch, test_loader):
    ...
    for data_sound, data_vis, target in test_loader:
        data_sound = torch.unsqueeze(data_sound,1) # convert 3 dimensional data to 4 dimensional data that model expects
        data_sound = torch.unsqueeze(data_sound,3)
        data_vis = torch.squeeze(data_vis)
        data_vis = torch.unsqueeze(data_vis,0)
        data_vis = data_vis.to(device)
        data_sound = data_sound.to(device)
        target = target.to(device).long() # pytorch wants target to be long() datatype
        output_obj, output_mat = model(data_vis, data_sound)
...
optimizer = optim.Adam(params, lr = 0.005) # change learning rate
...
torch.save(synth_model.state_dict(), '/content/gdrive/My Drive/phys101_synth.pth') # save oursound_model weights and biases
